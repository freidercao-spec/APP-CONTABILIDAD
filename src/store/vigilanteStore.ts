...
// ─── UTILERÍAS DE SEGURIDAD PARA OPERACIONES EN TABLAS SECUNDARIAS ───────────
const safeInsertTable = async (tableName: string, payload: any) => {
    try {
        const { error } = await supabase.from(tableName).insert(payload);
        if (error) {
            console.warn(`[Coraza] Registro omitido en ${tableName}:`, error.message);
        }
    } catch (err: any) {
        console.warn(`[Coraza] Excepción al escribir en ${tableName}:`, err.message || err);
    }
};

const safeUpdateTable = async (tableName: string, payload: any, matchField: string, matchValue: any) => {
    try {
        const { error } = await supabase.from(tableName).update(payload).eq(matchField, matchValue);
        if (error) {
            console.warn(`[Coraza] Actualización omitida en ${tableName}:`, error.message);
        }
    } catch (err: any) {
        console.warn(`[Coraza] Excepción al actualizar en ${tableName}:`, err.message || err);
    }
};
...
function mapDbToVigilante(row: any, historial: any[], descargos: any[], vacacion: any): Vigilante {
    return {
        id: row.codigo || `C-${row.id.substring(0, 4).toUpperCase()}`,
        dbId: row.id,
...
export const useVigilanteStore = create<VigilanteState>()(
    (set, get) => ({
            ...
            addVigilante: async (nombre, cedula, rango, modulo, justificacion, assignment) => {
                ...
                try {
                    const currentEmpresaId = useAuthStore.getState().empresaId || EMPRESA_ID;
                    const nextNum = get().nextIdNumber || 1000;
                    const finalCodigo = `C-${String(nextNum).padStart(4, '0')}`;
                    const tempId = `C-PEND-${Date.now()}`;
                    const optVig: Vigilante = {
                        id: tempId,
                        dbId: undefined,
                        nombre: `${nombres} ${apellidos}`.toUpperCase(),
                        nombres,
                        apellidos,
                        cedula,
                        rango: rango || 'Vigilante',
                        estado: modulo === 'activo' ? 'activo' : 'disponible',
                        puestoId: assignment?.puestoId || undefined,
                    ...
                    // Optimistic Push
                    set(state => ({
                        vigilantes: [...state.vigilantes, optVig],
                        vigilanteMap: new Map(state.vigilanteMap).set(tempId, optVig)
                    }));

                    // Insert into Supabase - write only existing columns in table schema
                    ...
                    if (error) {
                        // Rollback on error
                        set(state => ({
                            vigilantes: state.vigilantes.filter(v => v.id !== tempId)
                        }));
                    ...
                    const codigo = inserted.codigo || finalCodigo;
                    logAction('VIGILANTES', 'Registro Inicial', `Efectivo ${codigo} (${nombre}) incorporado al sistema.`, 'success');

                    // Insert initial historial safely (table might not exist in simplified DB)
                    ...
                    const newVig: Vigilante = mapDbToVigilante(inserted, [], [], undefined);
                    
                    // Replace optimistic with real
                    set(state => ({
                        vigilantes: state.vigilantes.map(v => v.id === tempId ? newVig : v),
                        vigilanteMap: new Map(state.vigilanteMap).set(newVig.id, newVig).set(newVig.dbId!, newVig)
                    }));

                    // Silent refresh after update
                    get().fetchVigilantes().catch(() => {});
            ...
            updateGuardStatus: async (id, estado, puestoId, justificacion) => {
                ...
                // Write to Supabase - write only existing columns (no puesto_actual_id / justificacion_disponible in table schema)
                await supabase
                    .from('vigilantes')
                    .update({
                        estado,
            ...
            addDescargo: async (vigilanteId, descargo) => {
                ...
                if (vigilante?.dbId) {
                    ...
                    await safeInsertTable('descargos', {
                        empresa_id: currentEmpresaId,
                        vigilante_id: vigilante.dbId,
            ...
            resolverDescargo: async (vigilanteId, descargoId) => {
                ...
                await safeUpdateTable('descargos', { estado: 'resuelto' }, 'id', descargoId);
            ...
            setVacaciones: async (vigilanteId, inicio, fin, motivo) => {
                ...
                if (vigilante?.dbId) {
                    ...
                    await safeUpdateTable('vacaciones', { estado: 'cancelado' }, 'vigilante_id', vigilante.dbId);
                    
                    await safeInsertTable('vacaciones', {
                        vigilante_id: vigilante.dbId,
                        fecha_inicio: inicio,
                        fecha_fin: fin,
                        motivo: motivo || null,
            ...
            updateVigilante: async (id, updates) => {
                ...
                if (vigilante?.dbId) {
                    ...
                    if (updates.nombre) {
                        ...
                        dbUpdates.nombre = updates.nombre.toUpperCase();
            ...
            setupRealtime: () => {
                ...
                const existingChannel = supabase.getChannels().find(ch => ch.topic.includes('vigilantes-realtime'));
                if (existingChannel) {
                    supabase.removeChannel(existingChannel);
                }

                supabase
                    .channel('vigilantes-realtime')
                    .on(
                        'postgres_changes',
                        {
                            event: '*',
                        ...
                        async (payload) => {
                            console.log('[Realtime] Cambio detectado en vigilantes:', payload.eventType);
                            
                            if (payload.eventType === 'DELETE') {
                                set(state => ({
                                    vigilantes: state.vigilantes.filter(v => v.dbId !== payload.old.id)
                                }));
                            } else {
