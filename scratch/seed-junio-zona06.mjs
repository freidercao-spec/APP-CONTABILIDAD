/**
 * CORAZA CTA — Seed Programación JUNIO 2026 desde Excel (ZONA 06)
 * ===============================================================
 * Lee scratch/excel_parsed_zona06.json y carga la programación real
 * en la base de datos Supabase para ZONA 06.
 */
...
async function main() {
  ...
  const excelData = JSON.parse(readFileSync(path.join(__dirname, 'excel_parsed_zona06.json'), 'utf-8'));
  ...
  console.log(`👮 Vigilantes en BD: ${vigilantesDB.length}`);

  // Obtener el próximo código secuencial disponible
  const codes = vigilantesDB
    .filter(v => v.codigo)
    .map(d => {
      const m = d.codigo.match(/C-(\d+)/);
      return m ? parseInt(m[1], 10) : 0;
    });
  let nextCodeNumber = Math.max(...codes, 0) + 1;
  console.log(`📡 Próximo código de vigilante: C-${String(nextCodeNumber).padStart(4, '0')}`);
  ...
  // Restringir coincidencia de puesto a los de NUEVA_ZONA para evitar cruzamiento
  function buscarPuesto(cliente) {
    const puestosZona = puestosDB.filter(p => p.zona === NUEVA_ZONA);
    let mejorMatch = null, mejorSim = 0;
    for (const p of puestosZona) {
  ...
  for (const [pi, excelPuesto] of excelData.entries()) {
    console.log(`\n${'─'.repeat(60)}`);
    console.log(`📌 [${pi + 1}/${excelData.length}] ${excelPuesto.cliente}`);

    let puestoDB = buscarPuesto(excelPuesto.cliente);

    if (!puestoDB) {
      console.log(`   ⚠️  Puesto no encontrado en ${NUEVA_ZONA}. Creando...`);
      ...
      console.log(`   ✅ Puesto encontrado: ${puestoDB.nombre} [${puestoDB.id}]`);
    }
    ...
    for (const [gi, guarda] of excelPuesto.guardas.entries()) {
      ...
      if (!vigilante) {
        const newCode = `C-${String(nextCodeNumber++).padStart(4, '0')}`;
        console.log(`   ➕ Creando vigilante: ${guarda.nombre} (CC: ${guarda.cedula}) -> ${newCode}`);
    ...
    if (existing?.id) {
      ...
      console.log(`   🔄 Programación mensual actualizada: ${progId}`);
    } else {
      const { data: newProg } = await sb
        .from('programaciones_mensuales')
        .insert({ puesto_id: puestoDB.id, anio: ANO, mes: MES_INDEX, empresa_id: EMPRESA_ID, estado: 'published', version: 1, personal: personalJson })
        .select('id').single();
      progId = newProg.id;
      console.log(`   📋 Programación mensual creada: ${progId}`);
    }

    // Limpiar y reinsertar asignaciones
  ...
  console.log(`\n${'═'.repeat(62)}`);
  console.log(`🚀 SEED ZONA 06 COMPLETADO`);
