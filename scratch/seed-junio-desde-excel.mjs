/**
 * CORAZA CTA — Seed Programación JUNIO 2026 desde Excel
 * ======================================================
 * Lee scratch/excel_parsed.json y carga la programación real
 * en la base de datos Supabase.
 *
 * Tablas correctas (verificadas):
 *   programaciones_mensuales   → cabecera + campo personal (JSON)
 *   asignaciones_programacion  → un registro por día/rol
 *   puestos                    → puestos de trabajo
 *   vigilantes                 → guardas de seguridad
 *
 * Mapeo de códigos Excel → jornada/turno:
 *   D12  → normal / AM  · 06:00 - 18:00
 *   D8   → normal / AM  · 07:00 - 15:00
 *   D10  → normal / AM  · 06:00 - 16:00
 *   D14  → normal / AM  · 06:00 - 20:00
 *   N12  → normal / PM  · 18:00 - 06:00
 *   N14  → normal / PM  · 18:00 - 08:00
 *   NR   → descanso_no_remunerado
 *   VAC/Vc → vacaciones
 *   INC  → incapacidad
 *   CZ   → capacitacion
 *   SUP  → permiso
 *   X    → ausencia_no_justificada
 *   DIS  → disponible
 *   (blank) → descanso_remunerado
 */
...
function traducirCodigo(codigo) {
  ...
  // Número u otro código desconocido → descanso remunerado
  return { jornada: 'descanso_remunerado', turno: 'AM', inicio: null, fin: null };
}
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║  CORAZA CTA — Seed Programación Junio 2026 desde Excel  ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  // Cargar puestos de ZONA 20 de la BD
  ...
  function buscarVigilante(cedula, nombre) {
    const porCed = vigilantesPorCedula.get(String(cedula).trim());
    if (porCed) return porCed;
    const porNombre = vigilantesPorNombre.get(normalizarNombre(nombre));
    if (porNombre) return porNombre;
    let mejorMatch = null, mejorSim = 0;
    for (const [nomKey, vig] of vigilantesPorNombre) {
      const sim = similitud(nombre, nomKey);
      if (sim > mejorSim) { mejorSim = sim; mejorMatch = vig; }
    }
    if (mejorSim >= 0.6) return mejorMatch;
    return null;
  }
  ...
  for (const [pi, excelPuesto] of excelData.entries()) {
    ...
    if (!puestoDB) {
      console.log(`   ⚠️  Puesto no encontrado. Creando...`);
      const nombrePuesto = excelPuesto.cliente.trim();
      const { data: nuevoPuesto, error: nPErr } = await sb
        .from('puestos')
        .insert({ empresa_id: EMPRESA_ID, nombre: nombrePuesto, zona: 'ZONA 20', estado: 'activo' })
        .select('id, nombre')
        .single();
    ...
    todasAsignaciones.forEach(asig => { asig.programacion_id = progId; });
  ...
  console.log(`   ✅ ${totalProg} programaciones insertadas`);
  console.log(`   ✅ ${totalAsigs} asignaciones insertadas`);
  console.log(`   ➕ ${totalNuevosVigilantes} vigilantes nuevos creados`);

  if (guardasSinMatch.length > 0) {
    console.log(`\n   ⚠️  Guardas NO insertados (${guardasSinMatch.length}):`);
    guardasSinMatch.forEach(g => console.log(`      • ${g.nombre} (CC: ${g.cedula})`));
  }
  if (puestosSinMatch.length > 0) {
    console.log(`\n   ⚠️  Puestos NO procesados (${puestosSinMatch.length}):`);
    puestosSinMatch.forEach(p => console.log(`      • ${p}`));
  }
