...
console.log(`Parsing ZONA 23 sheet: "${sheet.name}"...`);

const puestosMap = new Map(); // puestoName -> { cliente: puestoName, puesto: 'PORTERIA', guardas: [] }
let lastPuesto = '';
...
for (let r = 8; r <= sheet.rowCount; r++) {
  ...
  // Skip header rows or metadata rows
  ...
  // Update lastPuesto if we find a valid puesto name, except if it is "RELEVANTE"
  ...
  if (gName && /^\d+$/.test(gCed)) {
    ...
    const dias = {};
    for (let d = 1; d <= 31; d++) {
      const cell = row.getCell(6 + d); // Day 1 is Column 7 (G)
...
writeFileSync('scratch/excel_parsed_zona23.json', JSON.stringify(puestos, null, 2));
console.log('Saved to scratch/excel_parsed_zona23.json');
