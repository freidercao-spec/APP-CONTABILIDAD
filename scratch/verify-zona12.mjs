/**
 * VERIFICACIÓN CRUZADA: Excel vs Base de Datos (ZONA 12)
...
const excelData = JSON.parse(readFileSync(path.join(__dirname, 'excel_parsed_zona12.json'), 'utf-8'));
...
// Cargar todos los puestos de BD
...
function buscarPuesto(cliente) {
  ...
  if (bestSim >= 0.8) return best;
  return null;
}

console.log('=== VERIFICACIÓN CRUZADA EXCEL vs BD (ZONA 12) ===\n');

for (const [pi, excelPuesto] of excelData.entries()) {
  const puestoDB = buscarPuesto(excelPuesto.cliente);
  if (!puestoDB) {
    console.log(`[${pi+1}] ❌ Puesto no encontrado en BD: ${excelPuesto.cliente}`);
    totalError += excelPuesto.guardas.length * 30;
    totalDias += excelPuesto.guardas.length * 30;
    continue;
  }

  // Obtener programación del mes (Junio = mes 5)
...
console.log(`\n${'═'.repeat(60)}`);
console.log(`RESULTADO FINAL ZONA 12:`);
