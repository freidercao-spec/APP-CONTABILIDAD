/**
 * CORAZA CTA — Verificación de Programación JUNIO 2026 (ZONA 04)
 * ================================================================
...
async function main() {
  ...
  console.log('\n' + '─'.repeat(70));
  console.log('DETALLE POR PUESTO (Muestra de los primeros 15):');
  ...
  for (const excelPuesto of excelData) {
    ...
    const cleanExcel = nombreExcel.toLowerCase().replace(/[^a-z0-9]/g, '');
    let puestoDB = puestosZona.find(p => p.nombre.toLowerCase().replace(/[^a-z0-9]/g, '') === cleanExcel);
    if (!puestoDB) {
      puestoDB = puestosZona.find(p => {
        const cleanDB = p.nombre.toLowerCase().replace(/[^a-z0-9]/g, '');
        return cleanDB.includes(cleanExcel) || cleanExcel.includes(cleanDB);
      });
    }

    if (!puestoDB) {
      console.log(`  ❌ NO ENCONTRADO EN BD: ${nombreExcel}`);
      puestosFail++;
      continue;
    }
    ...
    if (!progId) {
      console.log(`  ⚠️  SIN PROGRAMACIÓN EN BD: ${nombreExcel}`);
      puestosFail++;
      continue;
    }
