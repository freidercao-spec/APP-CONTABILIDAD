/**
 * CORAZA CTA — Verificación de Programación JUNIO 2026 (ZONA 06)
 * ================================================================
...
async function main() {
  ...
  console.log(`📦 Puestos en BD (${ZONA}): ${puestosZona ? puestosZona.length : 0}`);
  console.log(`📄 Puestos en Excel: ${excelData.length}\n`);

  if (!puestosZona || puestosZona.length === 0) {
    console.log('❌ No se encontraron puestos para esta zona en la BD.');
    return;
  }
  ...
  console.log(`📋 Programaciones en BD: ${programaciones ? programaciones.length : 0}`);
  ...
  const matchRate = expectedAsigs > 0 ? ((totalAsigs / expectedAsigs) * 100).toFixed(1) : '0.0';
  ...
  for (const excelPuesto of excelData) {
    ...
    const puestoDB = puestosZona.find(p => 
      p.nombre.toLowerCase().replace(/\s+/g, '') === nombreExcel.toLowerCase().replace(/\s+/g, '')
    );
